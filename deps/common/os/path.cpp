...
bool remove_directory(std::string &path)
{
  ...
  common::list_file(path.c_str(), nullptr, files);
  bool rm_state = true;
  for (auto f : files) {
    if (is_directory(f.c_str())) {
      if (!remove_directory(f)) {
        rm_state = false;
        LOG_ERROR("file directory %s remove failed.", f.c_str());
      }
    } else {
      if (0 != remove(f.c_str())) {
        rm_state = false;
  ...
  return (0 == remove(path.c_str())) && rm_state;
}
