...
class MiniObServer:
    ...
    def __check_base_dir(self, base_dir: str):
        if not (os.path.isdir(base_dir)):
            raise (
                Exception(
                    "failed to check base directory. "
                    + base_dir
                    + " is not a directory"
                )
            )
    ...
    def stop_server(self):
        ...
        _logger.info(
            "miniob-server exit with code %d. pid=%s",
            return_code,
    ...
    def __check_unix_socket_server(self):
        with socket.socket(socket.AF_UNIX, socket.SOCK_STREAM) as s:
            ...
            if errno == 0:
                ...
                _logger.debug(
                    "Failed to connect to server. err=%d:%s", errno, os.strerror(errno)
                )
                return False
...
class MiniObClient:
    ...
    def __init_tcp_socket(self, server_port: int):
        ...
        if errno != 0:
            _logger.error(
                "Failed to connect to server with port %d. errno=%d:%s",
                server_port,
                errno,
                os.strerror(errno),
            )
    ...
    def __init_unix_socket(self, server_socket: str):
        ...
        if errno != 0:
            _logger.error(
                "Failed to connect to server with address '%s'. errno=%d:%s",
                server_socket,
                errno,
                os.strerror(errno),
            )
    ...
    def __recv_response(self):
        ...
        while True:
            ...
            if event & (select.POLLHUP | select.POLLERR):
                msg = (
                    "Failed to receive from server. poll return POLLHUP(%s) or POLLERR(%s)"
            ...
            if len(data) > 0:
                ...
                _logger.debug(
                    "receive from server[size=%d]: '%s'", len(data), result_tmp
                )
...
class TestCaseLister:
    ...
    def list_directory(self, base_dir: str) -> List[TestCase]:
        ...
        if False == is_dir:
            raise (
                Exception(
                    "Failed to list directory while getting test cases. "
                    + base_dir
                    + " is not a directory"
                )
            )
...
class TestSuite:
    ...
    def set_test_result_tmp_dir(self, test_result_tmp_dir: str):
        ...
        if not (os.path.isdir(test_result_tmp_dir)):
            raise (
                Exception(
                    "Failed to set test result temp directory. "
                    + test_result_tmp_dir
                    + " is not a directory or failed to create"
                )
            )
        ...
        with open(file1, "r") as f1, open(file2, "r") as f2:
            ...
            for i in range(line_num):
                if lines1[i].upper() != lines2[i].upper():
                    _logger.info(
                        "file1=%s, file2=%s, line1=%s, line2=%s",
                        file1,
                        file2,
                        lines1[i],
                        lines2[i],
    ...
    def __run_case(self, test_case: TestCase) -> int:
        ...
        with open(result_tmp_file_name, mode="wb") as result_file:
            ...
            with CommandRunner(
                result_writer, self.__server_port, unix_socket
            ) as command_runner:
                ...
                for command_line in test_case.command_lines():
                    ...
                    if result is False:
                        _logger.error(
                            "Failed to run command %s in case %s",
                            command_line,
                            test_case.get_name(),
                        )
                        return result
    ...
    def run(self, eval_result: EvalResult):
            ...
            try:
                ...
                if result is Result.true:
                    ...
                    if result is Result.false:
                        ...
                        timeout_count += 1
                        eval_result.append_message(
                            "%s is timeout" % test_case.get_name()
                        )
            except Exception as ex:
        ...
        _logger.info(
            "All done. %d passed, %d failed, %d timeout",
            success_count,
            failure_count,
            timeout_count,
        )
    ...
    def __start_server_if_need(self, clean_data_dir: bool):
        ...
        if self.__need_start_server:
            ...
            miniob_server = MiniObServer(
                self.__db_server_base_dir,
                self.__db_data_dir,
                self.__db_config,
                self.__server_port,
                unix_socket,
...
def __init_options():
    ...
    # 当前miniob的代码目录
    options_parser.add_argument(
        "--project-dir", action="store", dest="project_dir", default=""
    )
...
def __init_test_suite(options) -> TestSuite:
    ...
    test_suite.set_test_case_base_dir(
        os.path.abspath(options.project_dir + "/test/case/test")
    )
    test_suite.set_test_result_base_dir(
        os.path.abspath(options.project_dir + "/test/case/result")
    )
    test_suite.set_test_result_tmp_dir(
        os.path.abspath(options.work_dir + "/result_output")
    )
...
def compile(
    ...
    if not os.path.exists(work_dir):
        _logger.error(
            "The work_dir %s doesn't exist, please provide a vaild work path.", work_dir
        )
        return False
    ...
    if os.path.exists(build_path) and rebuild_all:
        _logger.info(
            "build directory is not empty but will be cleaned before compile: %s",
            build_path,
        )
